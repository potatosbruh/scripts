...
function autoCraft()
    spawn(function()
        local parent = game:GetService("Workspace").Forging.anvil.Part
        while getgenv().craft == true do
                for i, v in next, GetDescendants(game:GetService("Workspace").Forging) do
...
function autoFarm()
    spawn(function()
        while getgenv().farm == true do
                local Tools = filter(GetDescendants(Services.Workspace), function(i,v) -- line 222 and 26   
        return IsA(v, "Tool") and FindFirstChild(v, "Handle"); -- 60 and 61
    end)
    UnequipTools(GetHumanoid()); -- 157 and 1031
    local ToolAmount = #GetChildren(LocalPlayer.Backpack); -- 59
    for i, v in next, Tools do
        if (v.Handle) then
            firetouchinterest(v.Handle, GetRoot(), 0); --342 to 369
            wait();
...
function tpForge()
    easyTP(game:GetService("Workspace").Forging.Part.anvil.CFrame)
end
...
local Window = Library.CreateLib("World of Trollge", "Synapse")
...
home:NewLabel("use normal trolls while using this")
home:NewLabel("Credits to fates admin script")
home:NewLabel("Credits to r1ck#6969 for making most of this")



--------------------------------------------------------------------------------------- tab 1

sec1:NewToggle("Auto Farm", "This farms EVERYTHING (self explanatory)", function(state)
        getgenv().farm = state
        while state == true do
            autoFarm()
        wait(.1)
...
sec1:NewToggle("Auto Craft OIL", "This Crafts OIL", function(state)
        getgenv().craft = state
        tpForge()
        while state == true do
            autoCraft()
...
sec1:NewButton("Anti AFK (Credits to Warn)", "teleports to the forge", function()
...
sec2:NewButton("Oil Man Base", "teleports to the oil man place", function()
	trader()
end)
